import { matchesPayrollFilters, PayrollPaymentFilter } from "../../../../../common/types/payroll/payrollPayment.js";
...
export async function saveMonthlyPayrollReport(companyId: string, runId: string, department: string | null, paymentMethod: PayrollPaymentFilter = "all") {
  ...
  if (!["all", "bank", "cash"].includes(paymentMethod)) {
  ...
  const results = rows.filter((row) => matchesPayrollFilters(row, department, paymentMethod));
  ...
  const destination = await dialog.showSaveDialog({
    ...
    defaultPath: `rapport-paie-${run.year}-${String(run.month).padStart(2, "0")}-${safeDepartment}-${paymentMethod}.pdf`,
    filters: [{ name: "PDF", extensions: ["pdf"] }],
  });
  if (destination.canceled || !destination.filePath) return { canceled: true };
  const buffer = await renderToBuffer(<MonthlyPayrollReportDocument company={company} run={run} results={results} department={department} paymentMethod={paymentMethod} currency={settings?.currency ?? "BIF"} />);
