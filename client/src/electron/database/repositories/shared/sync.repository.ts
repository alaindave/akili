...
export async function addToSyncQueue(
  ...
  // Transaction callers notify pending changes only after their commit.
  ...
  const result = await (withinTransaction ? runDirect : run)(
    `
  ...
  if (!withinTransaction) await notifyPendingChanges(item.companyId);
